...
for i = 1:numel(quadcopters)
    pos = quadcopters(i).History.Position;
    xRange = max(pos(:, 1)) - min(pos(:, 1));
    yRange = max(pos(:, 2)) - min(pos(:, 2));
    xyRange = hypot(xRange, yRange);
    passed(i) = xyRange >= 150;
end

assert(mean(passed) >= 0.8, 'At least 80%% of quadcopters must move >= 150 m in XY.');

disp('testQuadcopterXYMovement passed.');
